...
export default function applySettings(settings: SettingStore, editor: Ace.Editor) {
	const darkTheme = resolveDarkMode(settings);

	editor.setOptions(getEditorOptions(settings, darkTheme));
...
export function getEditorOptions(
	...
	darkTheme =  resolveDarkMode(settings)
): Partial<Ace.EditorOptions> {
	return {
		enableBasicAutocompletion: settings.get('autocomplete'),
		enableLiveAutocompletion: settings.get('autocomplete'),
		...
		enableSnippets: settings.get('enableSnippets'),
